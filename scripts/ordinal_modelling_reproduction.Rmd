...
# experiment can be delayed,concurrent or all (for both experiment combined). test on all data to get more power.
...
# Both experiment are combined. The stimulus and task is the same in both
# concurrent and delayed confidence experiment for the reproduction data.
dat <-
  bind_rows(read_csv(here(sprintf('data/processed/%s_predicted_curve_concurrent.csv', task_to_analyze)), col_types = cols()) %>% mutate(conf_type = 'concurrent'),
            read_csv(here(sprintf('data/processed/%s_predicted_curve_delayed.csv', task_to_analyze)), col_types = cols()) %>% mutate(conf_type = 'delayed'))

# Renumber participants. In both experiments participant numbers are counted from 0 so there will be overlapping in numbers.
dat <- dat %>% 
  group_by(conf_type, participant) %>% 
...
dat <-
  dat %>% 
  mutate(dev= abs(target_length - 400)) %>% 
...
# all bias direction effects have to be positive. this is added to all posteriors
all_positive_effect <- samp_out_a[, br_bd] > 0 & samp_out_a[, po_bd] > 0 & samp_out_a[, ml_bd] > 0
# prior for all effects being positive
all_positive_prior <- ml > 0 & po > 0 & br > 0
  
# Priors H-I --------------------------------------------------------
prior_prob_HH <- mean(br > ml & ml < po & br < po & all_positive_prior)
prior_prob_II <- mean(br > ml & ml < po & br > po & all_positive_prior)

# Model HH (BR > ML < PO) & PO > BR
post_prob_conHH <- 
  mean(
    ba_bigger_ml & pa_bigger_ml & pa_bigger_ba & all_positive_effect
  )
...
prior_prob_JJ <- mean(br < ml & ml > po & br < po & all_positive_prior)
prior_prob_KK <- mean(br < ml & ml > po & br > po & all_positive_prior)
...
prior_prob_LL <- mean(br < ml & ml < po & br < po & all_positive_prior)
...
prior_prob_MM <- mean(br > ml & ml > po & br > po & all_positive_prior)
...
# Priors
prior_prob_BB <- mean(ml < po & br < po & all_positive_prior)
prior_prob_CC <- mean(ml > po & br > po & all_positive_prior)
...
post_prob_conBB <-  
  mean(
    pa_bigger_mlbr & samp_out_c[,mlbr_bd] > 0 & samp_out_c[,po_bd] > 0
  )
...
post_prob_conCC <-  
  mean(
    mlbr_bigger_pa & samp_out_c[,mlbr_bd] > 0 & samp_out_c[,po_bd] > 0
  )
...
# Priors
prior_prob_DD <- mean(ml < br & po < br & all_positive_prior)
prior_prob_EE <- mean(ml > br & po > br & all_positive_prior)
...
post_prob_conDD <-  
  mean(
    ba_bigger_mlpa & samp_out_d[, mlpo_bd] > 0 & samp_out_d[,br_bd] > 0
  )
...
post_prob_conEE <-  
  mean(
    mlpa_bigger_ba & samp_out_d[, mlpo_bd] > 0 & samp_out_d[,br_bd] > 0
  )
...
post_prob_conG <-  
  mean(
    samp_out_g[, ml_bd] > 0 & samp_out_g[, br_bd] > 0 
  )
...
post_prob_conF <-  
  mean(
    samp_out_h[, ml_bd] > 0 & samp_out_h[, po_bd] > 0 
  )
...
post_prob_conE <-  
  mean(
    samp_out_i[, br_bd] > 0 & samp_out_i[, po_bd] > 0 
  )
...
# Priors
prior_prob_FF <- mean(br < ml &po < ml & br > 0 & ml > 0 & po > 0)
prior_prob_GG <- mean(br > ml &po > ml & br > 0 & ml > 0 & po > 0)
...
post_prob_conFF <-  
  mean(
    ml_bigger_bapa & samp_out_l[, brpo_bd] > 0 & samp_out_l[,ml_bd] > 0
  )
...
post_prob_conGG <-  
  mean(
    bapa_bigger_ml & samp_out_l[, brpo_bd] > 0 & samp_out_l[,ml_bd] > 0
  )
...
bf_B <- log(post_prob_conB/ prior_prob_B) + bf_out_j$bf 
# model C: effect only in Muller-Lyer
bf_C <- log(post_prob_conC/ prior_prob_C) + bf_out_e$bf
# model D: effect only in Base rate
bf_D <- log(post_prob_conD/ prior_prob_D) + bf_out_k$bf           
# model E: effect in Payoff and Baserate, no effect in Muller-Lyer
bf_E <- log(post_prob_conE/ prior_prob_E) + bf_out_i$bf           
# model F: effect in Payoff and Muller-Lyer, no effect in Baserate
bf_F <- log(post_prob_conF/ prior_prob_F) + bf_out_h$bf           
# model G: effect in Muller-Lyer and Baserate, no effect in Payoff
bf_G <- log(post_prob_conG/ prior_prob_G) + bf_out_g$bf           
